...
A GitHub Action that queries the [Backstage](https://backstage.io/) Software Catalog and exposes results as workflow outputs.

## Features

- 🔍 Filter by kind, namespace, name, owner, tags, lifecycle, system, type, or any raw Backstage filter expression
- 📄 Cursor-based pagination — fetches all matching entities automatically
- 🔐 Flexible authentication: bearer token, OAuth2 client credentials, custom header, or none
- ⚡ Native `fetch` with configurable timeout and exponential-backoff retry (429/5xx)
...
```yaml
- uses: owner/query-backstage-catalog@v1
  with:
...
- Confirm `oauth_token_url`, `oauth_client_id`, and `oauth_client_secret` are all set
